    ...
    def inference(self, step_num):
        ...
        if self.conf.test_mode == 'first':
            ...
            for step in tqdm(range(self.num_test_batch)):
                ...
                cl_vector.extend(self.sess.run(self.latten,feed_dict=feed_dict))
            y_prob = np.array(y_prob)
            y_pred = np.array(y_pred)
            cl_vector = np.array(cl_vector)
            np.save('/project/ece/roysam/lbai/output/savevector-classification/vector.npy', cl_vector)
