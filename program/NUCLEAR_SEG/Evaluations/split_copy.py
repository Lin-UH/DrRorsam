...
import cv2
print(sys.path)
ProjRoot = "F:/Project/DrRorsam/program/NUCLEAR_SEG"
sys.path.append("F:/Project/DrRorsam/program/NUCLEAR_SEG/supplement")
print(sys.path)
import datasets_utils as dt_utils

sys.path.append("F:/Project/DrRorsam/program/NUCLEAR_SEG/autoseg")
...
parser.add_argument('-l', "--label", required=False,
                    default="F:/Project/DrRorsam/static/type/detections/merged_labelmask.h5",
...
parser.add_argument('-i', '--img_LOC', required=False,
                    ...
                    default="F:/Project/DrRorsam/static/type/8channels/multiplex.tif",
                    help='multiplex.tif')
parser.add_argument('-f', '--fTable_LOC', required=False, type=str,
                    metavar="/path/to/fTable_merged.csv/",
                    default="F:/Project/DrRorsam/static/type/detections/fTable_merged.csv",
...
parser.add_argument('--thres', "-t", required=False,
                    ...
                    default=0.2,
                    help="the lower threshold of sparse coefficent of positive cell type, the smaller this value is , the more splitted cells ")
...
new_dict = objs_dict[["ID", "centroid_x", "centroid_y", "xmin", "ymin", "xmax",
                      "ymax"]]  # new dict for saving the splited featureTable in the end
...
CTypeNames_dic = {"NeuN": 3, "IBA1": 4, "Olig2": 5, "S100": 6,
                  "GFP": 7}  # cell type names and their channel id at multiplex
...
multiplex = multiplex * np.dstack(
    [wholelabel > 0] * multiplex.shape[2])  # foreground  = intersection( nuclear mask ,cell type)
# print(multiplex[2000:2500,2000:2500,:3].shape)
# print(multiplex[2000:2500,2000:2500,3:4].shape)
# cv2.imshow("image", multiplex[20000:20500,20000:20500,3:4]*255)
# cv2.waitKey()
...
if demo:  # use predifined threshold to save time
    ...
    CTypeParas_dic = {
        'NeuN': {'n_components': 3, 'thres_ratio': 1.3, 'min_size': 120, 'c': [0, 255, 0], 'thres': 14.950000000000001},
        ...
        'GFP': {'n_components': 5, 'thres_ratio': 0.7, 'min_size': 100, 'c': [255, 255, 0],
                'thres': 21.349999999999998}}
    nuclear_label_CHN = {}
    for c_name in list(CTypeNames_dic.keys()):
        ...
        nuclear_label_CHN[c_name] = (intensity_CHN_demo > CTypeParas_dic[c_name]["thres"])
        ...
        plt.tight_layout()
        plt.show()
        plt.savefig('F:/split_cells/'+c_name+'.png')
        print("here")
    ...
    if args.val_crops is not None:  # extract the coord ids by atlas
        ...
        for crop_coords in atlas_names["crop_img"]:
            ...
            # cv2.imshow("image", multiplex[coord_x:coord_x+100, coord_y:coord_y+100, :3] * 255)
            # cv2.waitKey()
    ...
    for coord_x, coord_y in zip(coord_x_ls, coord_y_ls):
        ...
        for c_name in CTypeNames_dic:
            ...
            # print(chn_crop.shape)
            # cv2.imshow("image", chn_crop)
            # cv2.waitKey()
    ...
    for c_name in list(CTypeNames_dic.keys()):
        ...
        # print(label_demo)
        thres12 = np.array([x_train[label_demo == s_order[-2]].max(),
                            x_train[label_demo == s_order[-1]].min()]).mean()  # thres is between last one and last2
        ...
        if args.vis:
            ...
            tiff.imsave(os.path.join(write_path, c_name + "_NuclearBorder.tif"),
                        skimage.img_as_ubyte(border_merged * 255))
...
#         cell_type_pixel_sum.append(
#             np.sum(test_crop * CHN_mask * mask_crop))  # valid pixel intensity in each individual channel
#     multiplex_masked = CTypemask_crop * np.dstack(
#         [mask_crop] * multiplex_crop.shape[2])  # foreground  = intersection( nuclear mask ,cell type)
#
#     cell_type_channels_masked = multiplex_masked[:, :, 3:]  # 5CHNs
#     X = cell_type_channels_masked.reshape(-1,
#                                           5)  # shape :  n_features(cell types) * n_components (multiplex_crop.shape[0])
...
#     phenotype = np.argmax(
#         cell_type_pixel_sum) + 1 if phenotype == 0 else phenotype  # in case not founded, use the maximun intensisy
#
#     debris_num_ls = []  # the number of debrits
#     [debris_num_ls.append(measure.label(cell_type_channels_masked[:, :, ci]).max()) for ci in
#      range(cell_type_channels_masked.shape[2])]
...
#         CHNmask_ls = []
#         for i, (x, y) in enumerate(zip(np.uint(blobs[:, 0]), np.uint(blobs[:,
#                                                                      1]))):  # blobs read from seed detection result (blobs_log) or seed table
...
#                 zip(seed_x, seed_y)):  # blobs read from seed detection result (blobs_log) or seed table
#             seeds_chn[x, y] = 1  # disks of seeds are 1
#         seeds_marker = morphology.binary_dilation(seeds_chn, morphology.disk(
#             diskR))  # sure forground (marked) is from blobs with same radius
...
#     hf.close()
#     splited_merged_border = segmentation.find_boundaries(wholelabel)
#     tiff.imsave(os.path.join(write_path, "merged_border_lcRemoved.tif"),
#                 skimage.img_as_ubyte(splited_merged_border * 255))
#
# t5 = time.time()
#
